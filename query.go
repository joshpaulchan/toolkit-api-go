// Package main provides a example schema and resolver based on Star Wars characters.
//
...
var humansById = make(map[graphql.ID]*human)
...
var droidsById = make(map[graphql.ID]*droid)
...
var starshipsById = make(map[graphql.ID]*starship)

func init() {
	for _, s := range starships {
		starshipsById[s.ID] = s
	}
}
...
func (r *Resolver) Hero(args struct{ Episode string }) *characterResolver {
	if args.Episode == "EMPIRE" {
		return &characterResolver{&humanResolver{humansById["1000"]}}
	}
	return &characterResolver{&droidResolver{droidsById["2001"]}}
}
...
func (r *Resolver) Character(args struct{ ID graphql.ID }) *characterResolver {
	if h := humansById[args.ID]; h != nil {
		return &characterResolver{&humanResolver{h}}
	}
	if d := droidsById[args.ID]; d != nil {
...
func (r *Resolver) Starship(args struct{ ID graphql.ID }) *starshipResolver {
	if s := starshipsById[args.ID]; s != nil {
...
func (r *humanResolver) Starships() *[]*starshipResolver {
	...
	for i, id := range r.h.Starships {
		l[i] = &starshipResolver{starshipsById[id]}
	}
	return &l
}
...
func resolveCharacter(id graphql.ID) *characterResolver {
	if h, ok := humansById[id]; ok {
		return &characterResolver{&humanResolver{h}}
	}
	if d, ok := droidsById[id]; ok {
		return &characterResolver{&droidResolver{d}}
	}
	return nil
}
